...
var raf;
var collided, right, right2, left2, up2, left, up, jumped2 = false, jumped = false, no_prev_colloid, cyloc, cxloc, theta;
var ai_collided;
var g = 3;
var t, dt = 0.2;
var time = 0, newTime;
...
var ai = {
  ...
  //   (this.x - this.radius) > (court_net.x + court_net.width)
  // (right2 && this.x < canvas.width - this.radius)
  update: function () {

    if (right2 && this.x < (court_net.x - this.radius)) {
      this.vel.x = 6;
      this.x += this.vel.x;
    }

    else if (left2 && (this.x - this.radius) > 0) {
      this.vel.x = -6;
      this.x += this.vel.x;
    }
    ...
    if (this.y > canvas.height && jumped2) {
      jumped2 = 0;
      this.y = canvas.height;
      this.vel.y = 0;
      this.t = 0;
    ...
    else if (this.y <= canvas.height && jumped2) {
      this.y = 0.5 * g * this.t * this.t + this.vel.y * this.t + canvas.height;
    }

    else if (up2 && this.y == canvas.height) {
      jumped2 = true;
      this.t = 0;
      this.vel.y = -25;
    }
...
function draw() {
  ...
  console.log(player_score.innerText)

  if (player_score.innerText >= 2) {
    ctx.fillStyle = 'white';
    ...
    ctx.fillText(ai.name + " победил!", canvas.width / 2, canvas.height / 2);
...
ai.name = new URLSearchParams(document.location.search).get('secondUsername');
